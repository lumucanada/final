---
title: "Factors like the satisfaction of the federal government under Justin Trudeau, feeling about Justin Trudeau, gender, age and the province living in influence the 2019 canadian deferal election"
author: "LU MU"
date: "2020/12/5"
output: pdf_document
---
...
## Abstract
Based on the 2019 Canadian election study (Phone survey) data set, we use five variables which are the satisfaction of the federal government under Justin Trudeau, feeling about Justin Trudeau, gender, age and the province living in to investigate whether these factors influence a person choice of voting the Liberal Party or the Conservative Party. In order to accomplish it, We use two logistical regression models to analyze these factors and determine if they are the cases which affect people's vote choice for the Liberal Party or the Conservative Party. We get the conclusion that the four variables which are the federal government under Justin Trudeau, feeling about Justin Trudeau, gender and age affect a person's choice to vote for the Liberal while as four variables which are feeling about Justin Trudeau, gender, age and the province affect a person's choice to vote for the Conservative Party. Our results are crucial because we provide the information for both parties and tell them what factors have influences on their election. It is important for both parties to evaluate themselves and know their pros and cons so that we will have more chance to win the election. 


## Introduction

Canadian had the 2019 Canadian federal election on October 21, 2019. The Liberal Party lost the majority they had won in 2015 and won 157 seats to form a minority government incumbent by Prime Minister Justin Trudeau. The Conservatives got 121 seats by Andrew Scheer instead 95 seats in 2015, while the NDP lost 15 seats to end up with 24. 

I did research, and found out in Wikipedia. In a minority situation, governments must rely on the support of other parties to stay in power, providing less stability than a majority government. At the federal level, no minority government (excepting the odd case of the 14th) has lasted a standard four-year term. Most minority governments have lasted less than two years. The average duration of completed minorities in Canada is 479 days or approximately 1 year, 140 days counting only that part of the 14th Parliament that was a minority, or 1 year, 207 days counting the entire duration of it. 

According historical trend so I predict that there will be another Canadian federal election is coming in March, 2021. The Conservatives Party has changed party leader to Erin O'Toole after Andrew Scheer resigned as leader in late 2019; the leader of Liberal Party and the NDP remain same. Could Erin O’Toole beat Justin Trudeau to won more seats in Parliament or Justin Trudeau won the election back to majority government as in 2015. No one can tell, but we can predict by information and observation we get. 

There always so many factors which people who care can influence people’s choice of voting when it comes to federal election. Some people care about Environments, some care civic rights; Parents may more care about education, seniors could care more about benefit of retirement. Different provinces have their own issues; different genders think of different ways. I know some friend voted because the party leader who they like from leader’ personality or only the appearance of the leader.  

I use the 2019 Canadian Election results (phone survey) data set to do an investigation, find out different factors if could influence people’s choice to vote. I identified five variables that I thought which influence investigate. 

1.	Satisfaction of the federal government under Justin Trudeau
2.	Feel about Justin Trudeau
3.	Gender
4.	Age
5.	Province living in

I chose logistic regression to conduct the analysis in this report, those five variables above are categorical variables and I used dummy variable to conjoin in the logistic regression. I found out significance level of 0.05 from results by logistic regression.  I use the P-values of the different variables to verify if the probability of a person voter for the Liberal Party or the Conservatives Party be influenced. 

From the work I have done, the results which I got could be determined 2019 Canadian Election results (phone survey) data set is very useful, those information can be used for those Canadian parties to well prepare their policy to meet the interesting of the voters from different group. I am going to discuss more details regarding the data set and model in this report. 
...
Because the initial survey (the CPS) was conducted by telephone, the small proportion of households in Canada without landline or wireless telephones were excluded from the population.
...
three digits of the telephone number), and a suffix or “bank” (the last four digits of the number). 
...
$$
where p is the probability that a person votes liberal party, $\beta_0$ is the intercept, $\beta_{satisfactionofgovernment}$ is the parameter for the satisfaction of the federal government under Justin Trudeau, $\beta_{feelaboutJT}$ is the parameter for how people feel about Justin Trudeau, $\beta_{gender}$ is the parameter for gender and $\beta_{age}$ is the parameter for age and $\beta_{province}$ is the parameter for province people living in.
...
In our case, we want to estimate $\beta_{satisfactionofgovernment},\beta_{feelaboutJT},\beta_{gender},\beta_{age},\beta_{province}$ in:
$$
log(\frac{p}{1-p})=\beta_0+\beta_{satisfactionofgovernment}x_{satisfactionofgovernment}+\beta_{feelaboutJT}x_{feelaboutJT}+\beta_{gender}x_{gender}
$$
...
Thirdly, the old people who are 65+ tend to vote for the Conservative party or the Liberal Party than the young who is 18-24. The coefficient of 65+ is 0.5962 in the first model. 0.5962 means that people who are 65+ is more likely to vote for the Liberal Party. The coefficient of 65+ is 0.8330 in the second model. 0.8330 means that people who are 65+ is more likely to vote for the Conservative Party. This is quiet interesting because we get the conclusion that the old are more likely to vote for the Liberal Party and the Conservative party than the young. But this is not the case in the real life. The reality is that the young mostly support the Liberal and the old are more likely to vote for the Conservative Party. There are two main possibilities behind the fact that people are more likely to vote Conservative as they get older: The first is that the ageing process makes people more conservative, or because older people have different lifestyles, and therefore different priorities when voting. But it’s also possible that it’s not a person’s age that is important, but rather which generation they belong to. Older generations of voters, who were brought up in different circumstances to younger voters, could vote differently as a result. The fact that the young are more likely to vote for the Liberal is that younger generations are more socially liberal. This is partially because younger generations are more likely to have been to university (about 35% of 30-year-olds have a degree, compared with 10% of 70-year-olds) and higher education tends to make people more socially liberal on issues such as crime and immigration. 
...
### Figure 3
```{r, tidy =FALSE, echo =FALSE, warning= FALSE}
display_data_3 <- survey_data_2 %>% #select smaller subset fo data 
  select(
    q2
  )
Figure3 <- display_data_3 %>%
...
### Figure 4
```{r, tidy =FALSE, echo =FALSE, warning= FALSE}
display_data_4 <- survey_data %>% #select smaller subset fo data 
  select(
    q13
  )
#Shorten the responses 
Figure4 <- display_data_4 %>% #Create an overall view of family income for the respondents 
...
### Figure 5
```{r, tidy =FALSE, echo =FALSE, warning= FALSE}
display_data_5 <- survey_data_2 %>% #select smaller subset fo data 
  select(
    q20
  )
#Shorten the responses 
Figure5 <- display_data_5 %>% #Create an overall view of family income for the respondents 
...
## Weakness 
As we can see in the discussion, there are quiet a few weakness being exposed by using the 2019 CES phone survey data. We have seen that the frequencies of respondents by provinces do not match the percentage of population in each province and the frequency of female respondents is less than male in the result but the 2015 election has seen the majority of the respondents are female. Those conflicts are caused by a lack of sufficient sample size since the sample size is 4021 and some of them is even dropped due to no-response making the sample size even smaller. The lack of sample size will not indicate the real result and give the reader incorrect information. However, if we survey more respondents and increase the sample size, it will give us a more accurate result when it comes to the election. 
...
- Wikipedia, "Federal minority governments in Canada" from
https://en.wikipedia.org/wiki/Federal_minority_governments_in_Canada
- Wikipedia, "2019 Canadian federal election" from
https://en.wikipedia.org/wiki/2019_Canadian_federal_election
- Wikipedia, "Erin O'Toole" from
